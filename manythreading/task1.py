...
# """
# Одной из причин Deadlock'а в этом коде - попытка захватить два
# Lock-а вместе через кортеж with (self.lock, friend_bank.lock).
# Это приводит к тому, что два потока могут попытаться захватить друг друговы Lock-и
# одновременно, что вызовет Deadlock.
# Чтобы исправить эту ошибку, нужно сначала захватить свой Lock,
# а потом уже захватить Lock другого банка:
# """
